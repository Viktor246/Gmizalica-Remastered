...
if (!isset($_SESSION["ID"])) {
  ...
  include "baza.php";
	...
	$id = $_SESSION["ID"];
	$sql = "SELECT * FROM player JOIN bckg_clr ON player.bckg_seting = bckg_clr.bckgID JOIN snake_clr ON player.snake_seting = snake_clr.snakeclrID WHERE userID = '$id'";
...
function loop() {
  ...
  context.fillStyle = '#<?php
		switch ($row["snake_seting"]) {
			case "2":
				echo "$row['snake_color']";
			break;
			case "1":
				echo "$row['snake_color']";
			break;
